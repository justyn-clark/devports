...
Installs devports after downloading a release archive and verifying SHA256.
...
validate_archive_paths() {
  local archive="$1"
  local list_file="$TMP_DIR/archive-entries.txt"
...
OS="$(os_name)"
ARCH="$(arch_name)"
ASSET_NAME="devports-${TAG}-${OS}-${ARCH}.tar.gz"
...
if [ -z "$CHECKSUMS_URL" ]; then
  echo "error: release ${TAG} is missing ${CHECKSUMS_NAME}" >&2
  exit 1
fi
...
if [ -z "$EXPECTED_CHECKSUM" ]; then
  echo "error: could not find checksum entry for ${ASSET_NAME}" >&2
  exit 1
fi
...
BIN_PATH="$(find_binary "$EXTRACT_DIR" devports || true)"
if [ -z "$BIN_PATH" ]; then
  echo "error: extracted archive did not contain devports binary" >&2
  exit 1
fi
...
if [ -w "$INSTALL_DIR" ]; then
  install -m 0755 "$BIN_PATH" "$INSTALL_DIR/devports"
else
  sudo install -m 0755 "$BIN_PATH" "$INSTALL_DIR/devports"
fi

echo "Installed devports ${TAG} to $INSTALL_DIR/devports"
